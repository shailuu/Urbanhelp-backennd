...
const { generatePaymentConfirmationEmail } = require("../utils/emailTemplates"); // Import email template
...
const updateApprovedBookingPaymentStatus = async (req, res) => {
  try {
    const { id } = req.params; // Booking ID from the URL
    const { isPaid } = req.body; // isPaid boolean from the request body

    // Validate input
    if (typeof isPaid !== 'boolean') {
      return res.status(400).json({
        success: false,
        message: "Invalid input: 'isPaid' must be a boolean.",
      });
    }

    // Find the approved booking and update its payment status
    const updatedBooking = await ApprovedBooking.findByIdAndUpdate(
      id,
      { isPaid },
      { new: true, runValidators: true } // `new: true` returns the updated doc, `runValidators` ensures model validations run
    ).populate("service", "title").populate("approvedWorker", "name"); // Populate for email and notification
...
    // --- Start of new email and notification logic ---
    // Only send email and create notification if the status is set to paid (true)
    if (isPaid) {
      try {
        await sendEmail({
          to: updatedBooking.clientInfo.email,
          subject: "Payment Confirmed for Your Booking!",
          html: generatePaymentConfirmationEmail(updatedBooking, "Admin Manual Confirmation"),
        });
        console.log(`Payment confirmation email sent to ${updatedBooking.clientInfo.email}`);
      } catch (emailError) {
        console.error("[Payment Confirmation Email Error (Admin Update)]:", emailError);
        // Continue with the response even if email fails, as the payment status is updated
      }

      try {
        await Notification.create({
          userEmail: updatedBooking.clientInfo.email,
...
      } catch (notifError) {
        console.error("[Payment Confirmation Notification Error]:", notifError);
      }
    }
    // --- End of new email and notification logic ---
...
    });
  } catch (error) {
    console.error("[Update Approved Booking Payment Status Error]:", error);
    res.status(500).json({
      success: false,
